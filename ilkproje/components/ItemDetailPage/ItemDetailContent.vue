    ...
    <div class="d-flex flex-column justify-content-start align-items-start mt-3 ">
        ...
        <div id="cart-add" class="align-self-stretch row align-items-center mt-4">
            ...
            <div class="col-1 mx-2 ">
                <img src="../../static/images/heart2.png" class="pointer heart-custom" alt="">
            </div>
        </div>
...
<script>
import { convertItemToCartObject } from "../Helper/helperFunctions"
    ...
    methods: {
        ...
        addToCart() {
            if(this.currentSize === "None") {
                ...
                let ItemSepet = Object.assign({}, this.itemDetails);
                ItemSepet.chosenColor = this.currentColorObject.name;
                ItemSepet.chosenColorCode = this.currentColorObject.color;
                ItemSepet.chosenSize = this.currentSize;
                ItemSepet = convertItemToCartObject(ItemSepet);
                if(localStorage.getItem("user") !== null) {
                    let user = JSON.parse(localStorage.getItem("user"));
                    ItemSepet.userId = user.id;
                    this.$store.dispatch("addToCart", {item: ItemSepet, userId : user.id});
                }
                else {
                    this.$store.dispatch("addToCartWithoutFetch", ItemSepet);
                }
